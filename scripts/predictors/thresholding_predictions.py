...
def thresholding_predictions(scan, scan_dir, saros_mask_dir):
    """
    Function to make thresholding VAT predictions on CT scans

    Args:
        scan: The name of the image
        scan_dir: The directory containing the images.
        saros_mask_dir: The directory containing the SAROS masks.

    Returns:
        None. Saves processed images and KDE plots.
    """
    image_nii = nib.load(os.path.join(scan_dir, scan))
    ...
    for pred_name, (lower_thresh, upper_thresh) in threshold_ranges.items():
        ...
        vat_mask = (image_nii_data >= lower_thresh) & (image_nii_data <= upper_thresh) & saros_abd_cav_mask

        # Create NIfTI image from the mask
        pred_nii = nib.Nifti1Image(vat_mask.astype(np.int16), image_nii.affine)
